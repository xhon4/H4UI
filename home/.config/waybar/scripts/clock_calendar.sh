#!/bin/bash
# ┌─ H4UI ──────────────────────────────────────────────
# │ QUÉ ES:        arma el texto y el tooltip del reloj de waybar — la
# │                 hora en la barra, y un calendario + eventos de Google
# │                 Calendar (si tenés gcalcli) al pasar el mouse.
# │ PODÉS CAMBIAR: los colores ACCENT/BLUE/WHITE/etc. de más abajo (son
# │                 hex sueltos, no salen de palette.toml todavía), y
# │                 MAX_LINE si querés líneas de evento más largas/cortas.
# │ NO TOQUES:     el bloque de Python (indentación y f-strings) — un
# │                 espacio de más rompe el script entero.
# │ VA EN:         ~/.config/waybar/scripts/clock_calendar.sh
# │                 (lo llama el módulo "custom/clock" de config.jsonc)
# └──────────────────────────────────────────────────────
    ...
    try:
        ...
        if out:
            ...
            for line in clean_out.splitlines():
                ...
                events_text.append(f'<span foreground="#565658" size="smaller">{safe_line}</span>')
    except Exception:
        pass
...
# 👉 colores del tooltip del calendario (independientes de palette.toml)
ACCENT       = "#00cde6"
ACCENT_DARK  = "#0757c8"
BLUE         = "#168cff"
WHITE        = "#ffffff"
...
for i, line in enumerate(raw_cal.splitlines()):
    ...
    if i == 0:
        result_cal.append(f'{pad}<span foreground="{ACCENT}"><b>{line.strip().center(20)}</b></span>')
    elif i == 1:
        result_cal.append(f'{pad}<span foreground="{DIM}"><b>{line}</b></span>')
    else:
        highlighted = re.sub(
            r'(?<!\d)(' + str(now.day) + r')(?!\d)',
            f'<span foreground="{BG}" background="{ACCENT}"><b>\\1</b></span>',
            line,
            count=1
        )
